...
class Host:
    ...
    def create_message(self, src_address, dest_address, message_size,
        ...
        message = Message.Message(message_id, src_address, dest_address,
                                  message_size, schedule_time)

        self.total_sent += message.message_size
        self.packets_sent += 1
        if print_flag:
            print(
                f'Host <{self.address}> created an L2 Message with destination <{message.dst_address}> (size:<{message.message_size}>) at time <{message.schedule_time}>')

        return message

    def receive_message(self, message, link, print_flag=False, curr_time=0,time_to_receive=0):
        if message.message_type == "Data":
            if message.dst_address == self.address:
                self.total_rec += message.message_size
                self.packets_rec += 1
                self.last_receive_time = time_to_receive
                if print_flag:
                    print(f'Host <{self.address}> destroyed an L2 Message (size:<{message.message_size}>) '
                          f'at time <{self.last_receive_time}>')
                del message

    def create_timeline(self, number_of_packets):
        timeline = np.random.exponential(2, size=number_of_packets)
